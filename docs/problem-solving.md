...
## 2026-09-26：无密钥时验证异常路径

- 问题：尚不能用真实 API 测试，但第四阶段需要确认工具出错和模型调用失败时的行为。
- 处理：脚本化 LLM 返回除零及坏参数工具调用，检查 SQLite trace 和回传给模型的错误输出；用可取消 HTTP Transport 模拟请求超时，并用本地 HTTP 服务返回坏 JSON、缺字段和未完成响应。
- 验证：相关自动化测试及 `go test ./...` 全部通过；真实 API 冒烟测试仍待用户配置密钥。
